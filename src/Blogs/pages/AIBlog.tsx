...
>
  {avatarURL || auth.currentUser?.photoURL ? (
    ...
    <User className="w-5 h-5 cursor-custom-pointer" />
          ...
          <motion.button
            className={`px-6 hidden md:block hover:px-8 text-[15px] transition-all py-2 rounded-full font-semibold ${isDark ? 'bg-white text-black hover:bg-gray-100' : 'bg-black text-white hover:bg-gray-900'} flex items-center gap-2`}
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
        ...
        {isMenuOpen && (
          <motion.div
            className={`absolute top-full right-6 w-64 -mt-5 border border-gray-300 rounded-2xl shadow-lg z-10 overflow-hidden transition-all transform origin-top-right ${isDark ? 'bg-gray-800' : 'bg-white'}`}
            ...
            <nav className="p-3">
              {[
                ...
                <Link
                  key={index}
                  to={item.href}
                  className={`block px-6 py-2.5 md:py-3 text-[15.5px] md:text-[15.5px] font-medium rounded-lg transition-all duration-300 ease-in-out hover:ml-1 hover:font-semibold ${
                    isDark ? 'hover:bg-gray-750' : 'hover:bg-gray-100'
                  }`}
